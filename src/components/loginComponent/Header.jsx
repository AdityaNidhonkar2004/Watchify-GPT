...
import { SUPPORTED_LAGUAGES } from "../../utils/constant";
import { changeLanguage } from "../../utils/store/lagnSlice";
import { Link } from "react-router-dom";
import logo from "../../utils/Watchifylogofinal.png";
...
const Header = () => {
  ...
  return (
    <div className=" absolute z-50 w-full  bg-gradient-to-b from-black">
        ...
        <div>
          <Link to="/browse">
            <img
              src={logo}
              alt="watchify"
              className=" w-[180px] md:w-[250px] mx-auto md:mx-0  pt-3 md:pt-2 "
            />
          </Link>
        </div>
          ...
          <div className="flex  justify-between">
            {GPTSearchStatus && (
              <select
                ...
                id=""
                className="sm:px-2 px-1 capitalize py-0.5 mr-4 text-sm outline-none rounded bg-cyan-950  text-cyan-600  border-cyan-600 border-2"
              >
                {SUPPORTED_LAGUAGES.map((data) => (
                  <option value={data} key={data} className="capitalize">
                    {data}
                  </option>
                ))}
              </select>
